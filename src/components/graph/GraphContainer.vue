...
// 布局配置 - 调整为适合动态添加的参数
const layoutConfig = {
  nodeRepulsion: -1500,  // 增强节点排斥力，减少重叠（原-1000）
...
const initD3 = () => {
  ...
  simulation = d3.forceSimulation()
    ...
    .force('collide', d3.forceCollide().radius(35).strength(0.7))  // 新增碰撞力，防止节点重叠，间接减少边交叉
  ...
  simulation.on('tick', () => {
    linkGroup.selectAll('.link')
      .attr('x1', d => calculateEndpoint(d, true).x)
      .attr('y1', d => calculateEndpoint(d, true).y)
      .attr('x2', d => calculateEndpoint(d, false).x)
      .attr('y2', d => calculateEndpoint(d, false).y)
...
// 计算边的端点位置，使其从节点边界开始/结束
function calculateEndpoint(d, isSource) {
